...
begin
        ...
        if dmem_wr = '1' then
            case dmls_dtype_i is
                when LSU_BYTE  =>
                    case addr_base is
                        when b"00" =>
                            data_dat_o  <= dmst_data_i;
                            data_sel_o <= b"0001";
                        when b"01" =>
                            data_dat_o  <= dmst_data_i(23 downto 0) & dmst_data_i(31 downto 24);
                            data_sel_o <= b"0010";
                        when b"10" =>
                            data_dat_o  <= dmst_data_i(15 downto 0) & dmst_data_i(31 downto 16);
                            data_sel_o <= b"0100";
                        when b"11" =>
                            data_dat_o  <= dmst_data_i(7  downto 0) & dmst_data_i(31 downto  8);
                            data_sel_o <= b"1000";
                        when others =>
                            data_dat_o  <= (others => '-');
                            data_sel_o <= (others => '-');
    ...
    data_adr_o <= dmls_addr_i(XLEN-1 downto 2) when dmls_en_i = '1' else (others => '0');

    dmld_fault_o <= data_err_i and dmem_rd;
    dmst_fault_o <= data_err_i and dmem_wr;

    data_cyc_o <= dmrd_en or dmwr_en;
    data_stb_o <= dmrd_en or dmwr_en;
    data_we_o  <= dmwr_en;
